...
Imagine that we want to build two adjacent triangles, sharing two vertices. One way we can do it, is by creating six vertices in vertex buffer - `{v0, v1, v2}` for one triangle, and `{v3, v4, v5}` for another one. However we can see that `v5` is similar to `v0` and `v4` is similar to `v2`, which means we can be duplicating all the data they store like position, normal, UV, etc.
